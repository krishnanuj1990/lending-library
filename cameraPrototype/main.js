  ...
  decode: {
    ...
    anuj: {
      ...
      receiveMessage: function(e) {
        ...
        barcode_scanner.decode.anuj.workerCount--;
        if(e.data.success){
          var tempArray = e.data.result;
          for(var i = 0; i < tempArray.length; i++) {
            if(barcode_scanner.decode.anuj.resultArray.indexOf(tempArray[i]) == -1) {
              barcode_scanner.decode.anuj.resultArray.push(tempArray[i]);
            }
          }
          barcode_scanner.result.show(barcode_scanner.decode.anuj.resultArray.join("<br />"));
          barcode_scanner.decode.anuj.workerCount = 0;
        }else {
          if(barcode_scanner.decode.anuj.workerCount == 1) {
            var FlipWorker = new Worker("DecoderWorker.js");
            FlipWorker.onmessage = barcode_scanner.decode.anuj.receiveMessage;
            FlipWorker.postMessage({pixels: ctx.getImageData(0,0,canvas.width,canvas.height).data, cmd: "flip"});
          }
        }
        if(barcode_scanner.decode.anuj.workerCount == 0){
          if(barcode_scanner.decode.anuj.resultArray.length === 0) {
...
jQuery(function ($) {
  ...
  function scan() {
    ...
    barcode_scanner.decode.anuj.do(canvas);
